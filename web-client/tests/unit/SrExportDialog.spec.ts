...
describe('SrExportDialog.vue', () => {
    ...
    it('exports CSV (file picker path) and maps srcid → granule header + lookup', async () => {
        const cap = enableFilePickerCapture();

        // 🔁 fresh readRows for this test + logs
        duckQuery.mockImplementationOnce(async () => {
            const rows = [
            { a: 1, b: 'x', srcid: 42n },
            { a: 2, b: 'y', srcid: 99n },
            ];
            console.log('[CSV TEST] duckQuery called, returning fresh readRows');
            return {
            async *readRows(batch: number) {
                console.log('[CSV TEST] readRows invoked with batch =', batch);
                yield rows;
                console.log('[CSV TEST] readRows completed');
            },
            };
        });

        // open to trigger metadata watcher (fills headerCols)
        const wrapper = mountDialog({ modelValue: false });
        await wrapper.setProps({ modelValue: true });

        // ensure metadata present so header isn't empty
        const okMeta = await flushUntil(
            () =>
            duckCols.mock.calls.length > 0 &&
            Array.isArray((wrapper.vm as any).headerCols) &&
            (wrapper.vm as any).headerCols.length > 0,
            80
        );
        expect(okMeta).toBe(true);

        // select CSV
        await wrapper.find('[data-testid="formatSelect"]').setValue('csv');
        await nextTick();
        expect((wrapper.vm as any).selectedFormat).toBe('csv');

        // click Export
        await wrapper.findAll('button').at(1)!.trigger('click');

        const urlCreate = (globalThis as any).URL.createObjectURL as any;

        // Wait until we either:
        //  - see "granule-xyz" appear in the writer buffer, or
        //  - the object-URL fallback ran (we'll read the Blob), or
        //  - the writer closed after at least one write (and then we check text)
        const seenData = await flushUntil(() => {
            const txt = cap.readText();
            if (/granule-xyz/.test(txt) || /unknown_srcid_99/.test(txt)) return true;
            if (urlCreate?.mock?.calls?.length > 0) return true;
            if (cap.fsWritable.close.mock.calls.length > 0 && cap.writes.length > 0) return true;
            return false;
        }, 120);
        expect(seenData).toBe(true);

        // ---- DEBUG: dump what we captured
        const td = new TextDecoder();
        const decodedWrites = cap.writes.map((u8) => td.decode(u8));
        console.log('CSV writes (decoded):', decodedWrites);

        // Prefer file-picker path; otherwise read the Blob fallback
        let text = cap.readText();
        if (!text && urlCreate?.mock?.calls?.length) {
            const lastCall = urlCreate.mock.calls.at(-1);
            const blobArg = lastCall?.[0] as Blob;
            expect(blobArg).toBeInstanceOf(Blob);
            text = await blobArg.text();
            console.log('CSV fallback (object URL) text:\n---\n' + text + '\n---');
        } else {
            console.log('CSV assembled text:\n---\n' + text + '\n---');
        }

        const lines = text.trim().split(/\r?\n/);
        expect(lines[0]).toBe('a,b,granule');

        // Accept quoted or unquoted strings (safeCsvCell JSON-quotes strings)
        // Expected:
        //   1,"x","granule-xyz"
        //   2,"y","unknown_srcid_99"
        expect(text).toMatch(/(?:^|\n)1,?"?x"?,?"?granule-xyz"?(?:\r?\n|$)/);
        expect(text).toMatch(/(?:^|\n)2,?"?y"?,?"?unknown_srcid_99"?(?:\r?\n|$)/);

        // success toast fired
        const toastAdd = (await import('primevue/usetoast')).useToast().add;

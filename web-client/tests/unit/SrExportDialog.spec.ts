...
const defer = <T = any>() => {
  let resolve!: (_v: T | PromiseLike<T>) => void
  let reject!: (_e?: any) => void
      ...
      getDirectoryHandle: vi.fn().mockResolvedValue({
        getFileHandle: vi.fn().mockResolvedValue({
          getFile: vi.fn().mockResolvedValue(
            Object.assign(new File([new Uint8Array([1, 2, 3])], 'f.parquet'), {
...
describe.skip('SrExportDialog.vue', () => {
  ...
  it('exports GeoParquet via fetch stream pipeTo', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('/fake/export', expect.objectContaining({ method: 'POST' }))
    expect(toastAdd).toHaveBeenCalledWith(
      expect.objectContaining({ severity: 'success', summary: 'Export Complete' })
    )
  })

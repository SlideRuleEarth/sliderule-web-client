...
const storesMock = {
  useGeoJsonStore: () =>
    ({
      ...
      _debug: {
        get reqPoly() {
          return STATE.reqPoly
        },
        get feat() {
          return STATE.featGeoJSON
        }
      }
    }) as any,
...
vi.mock('jszip', () => ({
  default: {
    // eslint-disable-next-line @typescript-eslint/promise-function-async
    loadAsync: vi.fn((_buf: ArrayBuffer) => {
      const files = Object.fromEntries(
        Object.entries(STATE.zipFiles).map(([name, content]) => [
          name,
          {
            async: vi.fn((kind: 'string' | 'arraybuffer') => {
              if (kind === 'string') {
...
vi.mock('@/utils/SrMapUtils', () => ({
  // eslint-disable-next-line @typescript-eslint/promise-function-async
  handleGeoJsonLoad: vi.fn((_map: any, _geo: any, _opts: any) => Promise.resolve(STATE.mapExtent)),
...
describe('useReadShapefiles (Vitest)', () => {
  ...
  test('parseShapefileToGeoJSON: URL map uses fetch and returns GeoJSON', async () => {
    ...
    global.fetch = vi.fn((url: string) => {
      ...
      return Promise.resolve(mkResp(bytes))
    })
  ...
  test('readShapefileToOlFeatures: sourceCRS via prjToSupportedEpsg, reprojects when available', async () => {
    ...
    expect(warning).toBe(
      "Warning: The shapefile's .prj indicates a non-EPSG:4326 projection. This may cause misalignment."
    )
  })

...
const logger = createLogger('RequestMapStore')
...
/**
 * Request Map Store - Dedicated store for the Request view map (SrMap.vue)
 * This store maintains independent state from the Analysis map to prevent
 * projection/view conflicts and tile misalignment issues.
 */
export const useRequestMapStore = defineStore('requestMap', {
  state: () => ({
  ...
  actions: {
    setMap(mapInstance: OLMap) {
      ...
      logger.debug('Request map instance set')
    },
    ...
    getOrCreateGraticule(map: OLMap): Graticule {
      ...
      if (!graticule) {
        ...
        logger.debug('Created new graticule instance for request map', {
    ...
    setGraticuleForMap(targetMap?: OLMap) {
      ...
      this.graticules.forEach((graticule, map) => {
        if (!targetMap || map === targetMap) {
          ...
          logger.debug('Graticule visibility changed for request map', {
            visible: this.graticuleState,
    ...
    resetMap() {
      ...
      logger.debug('Request map reset to defaults')
    },
    ...
    setSelectedBaseLayer(baseLayer: string) {
      ...
      logger.debug('Request map base layer set', { baseLayer })
    },
    ...
    setSelectedView(view: string) {
      ...
      logger.debug('Request map view set', { view })
    },
    getSelectedView() {
      return this.selectedView
    },
    ...
    setCenterToRestore(center: number[]) {
      logger.debug('setCenterToRestore for request map', { center })
      if (center) {

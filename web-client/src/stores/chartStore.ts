...
interface ChartState {
  ...
  derivedArrayColumns: string[] // Columns derived from array aggregation/flattening
}
...
export const useChartStore = defineStore('chartStore', {
  ...
  actions: {
    ...
    hasMinMaxValues(reqIdStr: string, key: string): boolean {
      this.ensureState(reqIdStr)
      return !!(
        this.stateByReqId[reqIdStr]?.minMaxValues && this.stateByReqId[reqIdStr].minMaxValues[key]
      )
    },
    getMinValue(reqIdStr: string, key: string): number {
      this.ensureState(reqIdStr)
      if (
        this.stateByReqId[reqIdStr]?.minMaxValues &&
        this.stateByReqId[reqIdStr].minMaxValues[key]
      ) {
    ...
    getLow(reqIdStr: string, key: string): number {
      this.ensureState(reqIdStr)
      if (
        this.stateByReqId[reqIdStr]?.minMaxLowHigh &&
        this.stateByReqId[reqIdStr].minMaxLowHigh[key]
      ) {
    ...
    getElevationDataOptionForHeight(reqIdStr: string) {
      ...
      return this.stateByReqId[reqIdStr].elevationDataOptions[
        this.stateByReqId[reqIdStr].ndxOfElevationDataOptionsForHeight
      ]
    },
    ...
    setDerivedArrayColumns(reqIdStr: string, columns: string[]): void {
      this.ensureState(reqIdStr)
      this.stateByReqId[reqIdStr].derivedArrayColumns = columns
    },
    getDerivedArrayColumns(reqIdStr: string): string[] {
      this.ensureState(reqIdStr)
      return this.stateByReqId[reqIdStr].derivedArrayColumns
    },
    isDerivedColumn(reqIdStr: string, columnName: string): boolean {
      this.ensureState(reqIdStr)
      return this.stateByReqId[reqIdStr].derivedArrayColumns.includes(columnName)
    }
  }
})

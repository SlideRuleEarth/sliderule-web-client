...
/**
 * Calculate winding order for projected coordinates (x, y in meters).
 * Returns true if clockwise, false if counter-clockwise.
 */
function isClockwiseProjected(coords: Array<[number, number]>): boolean {
  let wind = 0
  for (let i = 0; i < coords.length - 1; i++) {
    wind += (coords[i + 1][0] - coords[i][0]) * (coords[i + 1][1] + coords[i][1])
  }
  return wind > 0
}
...
/**
 * Calculate the convex hull directly in the projection's native coordinates (meters for EPSG:5936).
 * This function uses Turf.js which works with any 2D coordinate system and properly handles winding order.
 *
 * @param projectedCoords - Array of [x, y] coordinates in the projection's native units (typically meters)
 * @returns Array of [x, y] coordinates representing the convex hull in counter-clockwise order
 */
export function convexHullInProjection(
  projectedCoords: Array<[number, number]>
): Array<[number, number]> {
  // Convert projected coordinates to Turf.js points
  // Turf.js treats these as [x, y] coordinates in any coordinate system
  const points = projectedCoords.map((coord) => point([coord[0], coord[1]]))
  const tfc = featureCollection(points)
  const hull = convex(tfc)
  ...
  // Extract the coordinates from the hull
  const coords: Array<[number, number]> = hull.geometry.coordinates[0].map(
    (coord: number[]) => [coord[0], coord[1]] as [number, number]
  )

  // Ensure counter-clockwise winding order
  if (isClockwiseProjected(coords)) {
    coords.reverse()
  }

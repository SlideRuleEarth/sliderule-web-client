  ...
  function handleUpload(event: FileUploadUploaderEvent) {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        logger.debug('Parsed GeoJSON data', {
          geoJsonData,
          uploadType: props.loadReqPoly ? 'request upload' : 'features upload'
        })
        ...
        // Set polygon source when loading from upload
        if (props.loadReqPoly && reqParamsStore.poly) {
          reqParamsStore.setPolygonSource('upload')
        }

        logger.debug('After handleGeoJsonLoad', { poly: reqParamsStore.poly })
      } catch (err: any) {
        logger.error('Error parsing or loading GeoJSON', {

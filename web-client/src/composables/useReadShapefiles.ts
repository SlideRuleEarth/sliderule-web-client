...
export async function loadShapefileToMap(
  ...
  if (toast) {
    toast.add({
      ...
      detail: warning
        ? `Parsed successfully. Note: ${warning}`
        : 'Parsed and displayed successfully.',
      life: toastLifeMs
    })
...
export async function readShapefileToOlFeatures(
  ...
  if (!haveTarget) {
    warning =
      (warning ? warning + ' ' : '') +
      `Target CRS ${targetCRS} is not registered; features will not be reprojected.`
  }
  ...
  logger.debug(
    `Converted GeoJSON to OL features: ${features.length} features, from ${sourceCRS} to ${targetCRS}`,
    { features }
  )
  return {
    features,

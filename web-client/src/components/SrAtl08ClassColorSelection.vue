<template>
  ...
  >
    ...
    <div
      ...
      v-for="(classValue, index) in atl08ClassColorMapStore.atl08ClassOptions"
      :key="index"
    >
      <div
        class="color-preview"
        :style="{
          backgroundColor: atl08ClassColorMapStore.getColorForAtl08ClassValue(classValue.value)
        }"
      ></div>
      <SrMenu
        ...
        :setSelectedMenuItem="
          (color: string) =>
            atl08ClassColorMapStore.setColorForAtl08ClassValue(classValue.value, color)
        "
        :getSelectedMenuItem="
          () => atl08ClassColorMapStore.getColorForAtl08ClassValue(classValue.value)
        "
        @update:modelValue="handleSelectionChanged(classValue.label, $event)"
      />
    </div>
  </Fieldset>
  <div v-else>Loading atl08ClassColorMap...</div>
</template>

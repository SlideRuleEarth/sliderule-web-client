...
const logger = createLogger('SrHelperGraticuleControl')
...
const isChecked = computed({
  get: () => helperStore.graticuleState,
  set: (value: boolean) => {
    helperStore.graticuleState = value
  }
})
...
function handleGraticuleChanged() {
  logger.debug('handleGraticuleChanged', { graticuleState: helperStore.graticuleState })
  helperStore.setGraticuleForMap()
}
</script>
...
<template>
  <div ref="graticuleControlElement" class="sr-graticule-control ol-unselectable ol-control">
    <div class="graticule-checkbox-container" title="Toggle Graticule">
      <input
        id="helper-graticule-toggle"
        v-model="isChecked"
        type="checkbox"
        class="sr-custom-checkbox"
        @change="handleGraticuleChanged"
      />
      <label for="helper-graticule-toggle" class="graticule-label">Grid</label>
    </div>
  </div>
</template>

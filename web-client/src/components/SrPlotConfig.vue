...
async function enableLocationFinder(): Promise<void> {
  ...
  if (newFields.length > 0) {
    logger.debug('enableLocationFinder: Adding fields to yDataOptions', {
      newFields,
...
watch(
  ...
  (loaded) => {
    ...
    if (api.includes('x')) {
      ...
      void srcIdTblStore.getUniqueSourceCount(props.reqId)
    }
  },
  { immediate: true }
)
    ...
    <div class="sr-select-gedi02a-el-options">
      ...
      <div class="sr-ged02ap-el-select" v-if="recTreeStore.selectedApi == 'gedi02ap'">
        <label class="sr-ged02ap-elevation-label" :for="`sr-ged02ap-elevation-field-select`"
          >Gedi02a Elevation to use</label
        >
        <Select

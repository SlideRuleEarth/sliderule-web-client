...
const emit = defineEmits<{
  (_e: 'features', _features: OLFeature<Geometry>[]): void
...
const onFilesSelected = (event: Event) => {
  ...
  void (async () => {
    try {
      ...
      const input =
        singleFile && singleFile.name.toLowerCase().endsWith('.zip') ? singleFile : files
      ...
      logger.error('Shapefile upload failed', {
  ...
  <div class="shp-upload">
    ...
    >
      <div class="picker">
        ...
        <!-- Hint -->
        <div class="hint">Accepts .zip, or .shp + .dbf + .shx + .prj</div>
      ...
      <template #footer>
        <Button label="Done" severity="secondary" @click="onCloseDialog" />
      </template>
    </Dialog>
  </div>
</template>
...
.shp-upload .file-input {
  width: 100%;
  margin-bottom: 1.5em;
}
:deep(.shp-upload-dialog) {
  min-width: 340px;
}
...
.picked {
  ...
  max-height: 7.5rem;
  overflow: auto;
  border-left: 2px solid var(--p-content-border-color, rgba(255, 255, 255, 0.12));
}
.picked li {
  font-size: 0.875rem;
  line-height: 1.4;

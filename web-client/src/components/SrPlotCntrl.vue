  ...
  <Fieldset :legend="computedLabel">
    <div class="sr-fieldset-box">
      ...
      <div class="sr-ydata-menu">
        <div>
          <label class="sr-y-data-label" :for="`srYColEncode-overlayed-${reqIdStr}`"
            >Point Color</label
          >
        ...
        <div
          class="sr-solid-color-selection-panel"
          v-if="chartStore.getSelectedColorEncodeData(reqIdStr) === 'solid'"
        >
          ...
          <div class="sr-solid-color-panel">
            ...
            <div
              class="sr-solid-color-preview"
              :style="{ backgroundColor: computedSolidSymbolColor }"
            ></div>
          </div>
        </div>
      </div>
    ...
    <div>
      <SrSymbolSize :reqIdStr="reqIdStr" />
    </div>
    <div>
      <SrRecIdReqDisplay
        :reqId="props.reqId"
...
import { EL_LAYER_NAME_PREFIX } from '@/types/SrTypes'
...
import {
  ...
  yDataSelectedReactive,
  yColorEncodeSelectedReactive,
  solidColorSelectedReactive,
  initializeColorEncoding,
  showYDataMenuReactive
...
const shouldDisplayGradientColorLegend = computed(() => {
  ...
  if (selectedColorEncodeData && selectedColorEncodeData !== 'unset') {
    if (func.includes('atl03') && selectedColorEncodeData !== 'solid') {
      if (
        selectedColorEncodeData !== 'atl08_class' &&
        selectedColorEncodeData !== 'atl24_class' &&
        selectedColorEncodeData !== 'atl03_cnf'
      ) {
        should = true
      }
    } else if (
      func.includes('atl06') ||
      func.includes('atl03x-surface') ||
      func.includes('atl03x-phoreal') ||
      func.includes('atl08') ||
      func.includes('atl24')
    ) {
...
const handleColorEncodeSelectionChange = async (event: SelectChangeEvent) => {
  ...
  // Also update the main elevation map if "Link to Plot" is enabled
  const globalChartStore = useGlobalChartStore()
  if (globalChartStore.enableLocationFinder) {
    const req_id = recTreeStore.selectedReqId
    if (req_id > 0) {
      await duckDbReadAndUpdateElevationData(req_id, EL_LAYER_NAME_PREFIX)
    }
  }
}

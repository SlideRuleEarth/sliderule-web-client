<template>
  <div class="defaults">
    ...
    <p v-else-if="defaultsStore.error" class="text-red-500">
      Error: {{ defaultsStore.error.message }}
    </p>
    ...
    <Accordion v-if="defaultsStore.defaults" v-model:value="topLevelActive" :multiple="false">
      ...
      <AccordionPanel v-for="section in sortedSections" :key="section.key" :value="section.key">
        <AccordionHeader>{{ section.header }}</AccordionHeader>
        <AccordionContent>
          <Accordion v-model:value="nestedActive[section.key]" :multiple="false">
            <AccordionPanel v-for="[key, value] in section.entries" :key="key" :value="key">
              ...
              <AccordionContent>
                ...
                <div v-else-if="Array.isArray(value)">
                  <pre class="text-xs whitespace-pre-wrap text-gray-600">
[{{ value.join(', ') }}]</pre
                  >
                </div>
                <div v-else>
...
const isFetched = computed(
  () => defaultsStore.fetched && !defaultsStore.loading && !defaultsStore.error
)
...
function copyToClipboard() {
  navigator.clipboard
    .writeText(formattedDefaults.value)

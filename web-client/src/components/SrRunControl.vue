...
const enableRunButton = computed(() => {
  return (
    ((serverStateStore.isAborting || serverStateStore.isFetching) && !props.includeAdvToggle) ||
    props.includeAdvToggle
  )
})
...
onMounted(() => {
  ...
  if (props.includeAdvToggle) {
    ...
    void requestsStore.setConsoleMsg(
      `Zoom in and select a geographic region (several sq Km).  Then click 'Run SlideRule' to start the process`
    )
  } else {
...
async function toggleRunAbort() {
  if (serverStateStore.isFetching) {
    ...
    logger.debug('Run button pressed calling processRunSlideRuleClicked', {
    ...
    if (props.includeAdvToggle) {
      try {
        ...
        logger.error('Error in processRunSlideRuleClicked', {
        ...
        >
          <div class="button-content">
            ...
            <div
              ...
              v-html="
                (serverStateStore.isFetching ? 'Abort' : props.buttonLabel).replaceAll('\n', '<br>')
              "
            ></div>
          </div>
        </Button>

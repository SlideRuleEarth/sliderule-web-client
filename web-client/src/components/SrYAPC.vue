...
const defaultEnableYAPC = () => {
  return reqParamsStore.enableYAPC !== undefined ? reqParamsStore.enableYAPC : false
}
const defaultYapc = () => {
  return useSlideruleDefaults().getNestedMissionDefault<object>(
    reqParamsStore.missionValue,
    'yapc'
  ) as Icesat2ConfigYapc
}
const ccvDefaultKnn = () => {
  return reqParamsStore.getUseYAPCKnn() !== undefined ? reqParamsStore.getUseYAPCKnn() : false
}
const ccvDefaultMinKnn = () => {
  return reqParamsStore.getUseYAPCMinKnn() !== undefined ? reqParamsStore.getUseYAPCMinKnn() : false
}
const ccvDefaultWindowHeight = () => {
  return reqParamsStore.getUsesYAPCWindowHeight() !== undefined
    ? reqParamsStore.getUsesYAPCWindowHeight()
    : false
}
const ccvDefaultWindowWidth = () => {
  return reqParamsStore.getUsesYAPCWindowWidth() !== undefined
    ? reqParamsStore.getUsesYAPCWindowWidth()
    : false
}
  ...
  <div class="sr-yapc-container">
    <div class="sr-yapc-header">
      <SrCheckBox
        ...
        :tooltipUrl="DOCS.icesat2.yapcClassification"
        labelFontSize="large"
      />
    </div>

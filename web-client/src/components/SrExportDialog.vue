...
const currentFormat = computed(() => {
  ...
  logger.debug('currentFormat computed', {
    reqId: props.reqId,
    isGeo,
    asGeoCache: fieldNameStore.asGeoCache
  })
...
watch(visible, async (val) => {
  if (val && props.reqId > 0) {
    try {
      ...
      logger.warn('Failed to pre-fetch metadata', {
...
const handleExport = async () => {
  if (!props.reqId || !selectedFormat.value) {
    logger.warn('handleExport called with invalid state — ignored', {
      reqId: props.reqId,
...
async function exportGeoParquet(fileName: string): Promise<boolean> {
  ...
  if (hasGeoMetadata) {
    ...
    const wb = await getWritableFileStream(
      `${baseFileName}_GEO.parquet`,
    ...
    try {
      ...
      logger.error('Failed to write GeoParquet file', {
        error: err instanceof Error ? err.message : String(err)
      })
      try {
        await wb.abort(err)

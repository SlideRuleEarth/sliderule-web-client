...
const defaultLength = () => {
  return (
    useSlideruleDefaults().getNestedMissionDefault<number>(reqParamsStore.missionValue, 'len') || 40
  )
}
const defaultStep = () => {
  return (
    useSlideruleDefaults().getNestedMissionDefault<number>(reqParamsStore.missionValue, 'res') || 1
  )
}
const defaultAlongTrackSpread = () => {
  return (
    useSlideruleDefaults().getNestedMissionDefault<number>(reqParamsStore.missionValue, 'ats') || 0
  )
}
const defaultMinimumPhotonCount = () => {
  return (
    useSlideruleDefaults().getNestedMissionDefault<number>(reqParamsStore.missionValue, 'cnt') || 1
  )
}

const ccvDefaultLength = () => {
  return reqParamsStore.getUseLength() !== undefined ? reqParamsStore.getUseLength() : false
}
const ccvDefaultStep = () => {
  return reqParamsStore.getUseStep() !== undefined ? reqParamsStore.getUseStep() : false
}
const ccvDefaultAlongTrackSpread = () => {
  return reqParamsStore.getUseAlongTrackSpread() !== undefined
    ? reqParamsStore.getUseAlongTrackSpread()
    : false
}
const ccvDefaultMinimumPhotonCount = () => {
  return reqParamsStore.getUseMinimumPhotonCount() !== undefined
    ? reqParamsStore.getUseMinimumPhotonCount()
    : false
}
    ...
    <div class="sr-ext-distance-container">
      <div class="sr-ext-distance-header-container">
        <SrMenuInput
          ...
          :tooltipUrl="DOCS.icesat2.photonExtent"
        />
      </div>

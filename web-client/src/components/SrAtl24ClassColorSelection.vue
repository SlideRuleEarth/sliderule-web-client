<template>
  <Fieldset
    v-if="atl24ClassColorMapStore"
    legend="Atl24 Class Colors"
    class="sr-legend-box"
    :toggleable="true"
    :collapsed="false"
  >
    ...
    <div
      class="sr-menu-container"
      v-for="(classValue, index) in atl24ClassColorMapStore.atl24ClassOptions"
      :key="index"
    >
      <div
        class="color-preview"
        :style="{
          backgroundColor: atl24ClassColorMapStore.getColorForAtl24ClassValue(classValue.value)
        }"
      ></div>
      <SrMenu
        ...
        :setSelectedMenuItem="
          (color: string) =>
            atl24ClassColorMapStore.setColorForAtl24ClassValue(classValue.value, color)
        "
        :getSelectedMenuItem="
          () => atl24ClassColorMapStore.getColorForAtl24ClassValue(classValue.value)
        "
        @update:modelValue="handleSelectionChanged(classValue.label, $event)"
      />
    </div>
  </Fieldset>
  <div v-else>Loading atl24ClassColorMap...</div>
</template>

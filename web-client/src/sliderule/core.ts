...
async function populateDefinition(rec_type: any): Promise<any> {
  if (rec_type in recordDefinitions) {
    ...
    try {
      ...
      logger.error('Failed to retrieve definition', {
        recType: rec_type,
...
function decodeElement(
  ...
  if (big_endian) {
    switch (type_code) {
      case INT8:
        return buffer.readInt8(byte_offset)
      case INT16:
        return buffer.readInt16BE(byte_offset)
      case INT32:
        return buffer.readInt32BE(byte_offset)
      case INT64:
        return buffer.readBigInt64BE(byte_offset) as bigint
      case UINT8:
        return buffer.readUInt8(byte_offset)
      case UINT16:
        return buffer.readUInt16BE(byte_offset)
      case UINT32:
        return buffer.readUInt32BE(byte_offset)
      case UINT64:
        return buffer.readBigUInt64BE(byte_offset) as bigint
      case BITFIELD:
        throw new Error(`Bit fields are unsupported`)
      case FLOAT:
        return buffer.readFloatBE(byte_offset)
      case DOUBLE:
        return buffer.readDoubleBE(byte_offset)
      case TIME8: {
        const rawval = buffer.readBigInt64BE(byte_offset) as bigint
        return new Date(Number(rawval / BigInt(1000000)))
      }
      case STRING:
        return String.fromCharCode(buffer.readUInt8(byte_offset))
      case USER:
        throw new Error(`User fields cannot be decoded as a primitive`)
      default:
        throw new Error(`Invalid field type ${type_code}`)
    }
  } else {
    switch (type_code) {
      case INT8:
        return buffer.readInt8(byte_offset)
      case INT16:
        return buffer.readInt16LE(byte_offset)
      case INT32:
        return buffer.readInt32LE(byte_offset)
      case INT64:
        return buffer.readBigInt64LE(byte_offset) as bigint
      case UINT8:
        return buffer.readUInt8(byte_offset)
      case UINT16:
        return buffer.readUInt16LE(byte_offset)
      case UINT32:
        return buffer.readUInt32LE(byte_offset)
      case UINT64:
        return buffer.readBigUInt64LE(byte_offset) as bigint
      case BITFIELD:
        throw new Error(`Bit fields are unsupported`)
      case FLOAT:
        return buffer.readFloatLE(byte_offset)
      case DOUBLE:
        return buffer.readDoubleLE(byte_offset)
      case TIME8: {
...
async function fetchAndProcessResult(
  ...
  try {
    ...
    if (stream) {
      ...
      while (loop_done === false) {
        ...
        if (value) {
          ...
          if (contentType == 'application/octet-stream') {
            ...
            while (bytes_to_process > 0) {
              ...
              if (!got_header && bytes_to_process > REC_HDR_SIZE) {
                ...
                decodeRecord(rec_type, buffer, rec_type_size, rec_size)
                  .then((result) => {
                    if (rec_type in callbacks) {
                      callbacks[rec_type](result)
                    } else {
                      logger.warn('No callback registered for record type', {
                        recType: rec_type,
                        result
                      })
                    }
                  ...
                  .catch((error) => {
                    decode_errors++
                    logger.error('Error decoding record', {
                      recType: rec_type,
          ...
          if (empty_chunks > 10) {
            loop_done = true
            logger.error('Too many empty chunks, stopping stream processing', {
        ...
        if (done) {
          ...
          results['decode_errors'] = decode_errors
      ...
      logger.debug('Stream processing summary', {
        recsCnt: recs_cnt,
        numChunksAppended: num_chunks_appended,
        results
      })
    ...
    // Handle any errors
    logger.error('Error fetching or processing stream', {
      error: error instanceof Error ? error.message : String(error)
    })
...
export async function source(
  ...
  // Replace 'any' with a more specific return type if possible
  ...
  try {
    ...
    logger.error('Error in fetchAndProcessResult', {
      url,
      options,
      error: error instanceof Error ? error.message : String(error)
    })
...
export function get_values(
  ...
  fieldtype: number
): Array<number | string | BigInt> {
  // Replace 'any' with a more specific type if possible
  let values: Array<number | string | BigInt> = []
  const buffer = Buffer.from(bytearray)
  switch (fieldtype) {
    case INT8:
      for (let i = 0; i < buffer.length; i += 1) values.push(buffer.readInt8(i))
      break
    case INT16:
      for (let i = 0; i < buffer.length; i += 2) values.push(buffer.readInt16LE(i))
      break
    case INT32:
      for (let i = 0; i < buffer.length; i += 4) values.push(buffer.readInt32LE(i))
      break
    case INT64:
      for (let i = 0; i < buffer.length; i += 8) values.push(buffer.readBigInt64LE(i))
      break
    case UINT8:
      for (let i = 0; i < buffer.length; i += 1) values.push(buffer.readUInt8(i))
      break
    case UINT16:
      for (let i = 0; i < buffer.length; i += 2) values.push(buffer.readUInt16LE(i))
      break
    case UINT32:
      for (let i = 0; i < buffer.length; i += 4) values.push(buffer.readUInt32LE(i))
      break
    case UINT64:
      for (let i = 0; i < buffer.length; i += 8) values.push(buffer.readBigUInt64LE(i))
      break
    case FLOAT:
      for (let i = 0; i < buffer.length; i += 4) values.push(buffer.readFloatLE(i))
      break
    case DOUBLE:
      for (let i = 0; i < buffer.length; i += 8) values.push(buffer.readDoubleLE(i))
      break
    case TIME8:
      for (let i = 0; i < buffer.length; i += 8) values.push(buffer.readBigInt64LE(i))
      break
...
export async function populateAllDefinitions(): Promise<any> {
  ...
  try {
    ...
    logger.error('Error populating record definitions', {
      error: error instanceof Error ? error.message : String(error)
    })

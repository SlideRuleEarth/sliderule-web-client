...
export async function checkDoneProcessing(  thisReqID:number,
    ...
    if((read_state === 'done_reading') || (read_state === 'error') || abortRequested){
        ...
        logger.debug('checkDoneProcessing status', { num_checks, num_post_done_checks, read_state, abortRequested, thisReqID, num_svr_exceptions, num_arrow_data_recs_processed, num_arrow_meta_recs_processed, target_numSvrExceptions, target_numArrowDataRecs, target_numArrowMetaRecs })
        if(got_all_cbs || abortRequested){
            ...
            if(abortRequested){ // Abort requested
                ...
                if(read_state === 'done_reading'){
                    logger.info('Processing completed successfully', { status_details, reqId: thisReqID, num_checks });
            ...
            logger.warn('Processing not yet complete', { num_checks, num_post_done_checks, read_state, abortRequested, thisReqID, num_svr_exceptions, num_arrow_data_recs_processed, num_arrow_meta_recs_processed, target_numSvrExceptions, target_numArrowDataRecs, target_numArrowMetaRecs });
        }
    } else {
...
onmessage = async (event) => {
    try{
        ...
        if (cmd.sysConfig){
            ...
            void errorMsg(reqID, { type: 'runWorkerError', code: 'WEBWORKER', message: 'cmd.sysConfig was not provided' });
            return;
        }
                    ...
                    if(result.filename.includes('_metadata.json')){
                        ...
                        if(num_arrow_metaFile_meta_recs_processed === 0){
                            try{
                                ...
                                logger.error('Failed to update request status to progress', { reqId: reqID, arrowCbNdx, error: error instanceof Error ? error.message : String(error) });
                            }
                        }
                        num_arrow_metaFile_meta_recs_processed++;
                        logger.debug('arrowrec.meta processing metadata.json', { arrowCbNdx, arrowMetaFilename, arrowMetaFileSize, arrowDataFileOffset, num_arrow_metaFile_meta_recs_processed });
                        ...
                        logger.debug('arrowrec.meta processing datafile', { arrowCbNdx, arrowDataFilename, arrowDataFileSize, arrowDataFileOffset, num_arrow_dataFile_meta_recs_processed });
                ...
                'arrowrec.data': async (result:any) => {
                    ...
                    if(result.filename.includes('_metadata.json')){
                        if(arrowMetaFile){
                            ...
                            logger.debug('arrowrec.data processing metadata.json', { arrowCbNdx, arrowMetaFileOffset, dataLength: result.data.length });
                        } else {
                            logger.error('arrowMetaFile was not initialized');
                        }
                    } else {
                        logger.debug('arrowrec.data writing to file', { arrowCbNdx, arrowDataFileOffset, dataLength: result.data.length, isArrayBuffer: (result.data instanceof ArrayBuffer || ArrayBuffer.isView(result.data)) });
                        ...
                        logger.debug('arrowrec.data write complete', { arrowCbNdx, arrowDataFileOffset, num_bytes, dataLength: result.data.length });
                        if (num_bytes !== result.data.length) {
                            logger.error('Failed to write all bytes to file', { num_bytes, dataLength: result.data.length });
                ...
                if(cmd.parameters){
                    ...
                    if(cmd.func){
                        ...
                        try{
                            ...
                            logger.debug('Fetching data', { func: cmd.func, reqId: reqID, parameters: JSON.stringify(cmd.parameters, null, 2) });
                            if(got_all_cbs){
                            ...
                            if(result){
                                ...
                                logger.debug('Done reading data', { func: cmd.func, read_result, target_numSvrExceptions, target_numArrowDataRecs, target_numArrowMetaRecs });
                            } else {
                                logger.error('Failed to get result from SlideRule', { func: cmd.func, reqId: reqID });
                            ...
                            while((num_retries_left > 0) && (target_numArrowDataRecs>0)){
                                logger.debug('Waiting for final callbacks', { num_retries_left, target_numArrowDataRecs, num_arrow_dataFile_data_recs_processed, num_arrow_metaFile_data_recs_processed });
                                if(
                                    ...
                                    logger.debug('Received all callbacks', { num_retries_left, target_numArrowDataRecs, num_arrow_dataFile_data_recs_processed, num_arrow_metaFile_data_recs_processed });
                                    got_all_cbs = true;
                                    break;
                                } else {
                                    logger.debug('Still waiting for callbacks (data)', { num_retries_left, target_numArrowDataRecs, num_arrow_dataFile_data_recs_processed, num_arrow_metaFile_data_recs_processed });
                                    logger.debug('Still waiting for callbacks (meta)', { num_retries_left, target_numArrowMetaRecs, num_arrow_dataFile_meta_recs_processed, num_arrow_metaFile_meta_recs_processed });
                                    logger.debug('Still waiting for callbacks (exceptions)', { num_retries_left, target_numSvrExceptions, num_svr_exceptions });
                                }
                                num_retries_left--;
                                await mysleep(1000);
                            }
                            if(!got_all_cbs){
                                logger.error('Failed to get all arrowrec.data callbacks', { num_retries_left, target_numArrowDataRecs, num_arrow_dataFile_data_recs_processed, num_arrow_metaFile_data_recs_processed });
                                //if(num_retries_left === 0) throw new Error('SlideRuleError:The server quit unexpectedly.');
                            }
                            logger.debug('Final callback counts', { target_numArrowDataRecs, num_arrow_dataFile_data_recs_processed, num_arrow_metaFile_data_recs_processed });
                            // Log the result to the console
                            logger.debug('Processing complete', { reqId: reqID });
                            logger.debug('Resource stats', { reqId: reqID, num_defs_fetched: get_num_defs_fetched(), num_defs_rd_from_cache: get_num_defs_rd_from_cache() });
                        } catch (error) {
                            read_state = 'error';
                            postMessage(progressMsg(reqID, 
                                {
                            ...
                            logger.error('Error reading data', { func: cmd.func, error: error instanceof Error ? error.message : String(error) });
                            let emsg = '';
                            emsg = String(error);
                            let code = '';
        ...
        logger.error('runWorkerError', { error: error instanceof Error ? error.message : String(error) });

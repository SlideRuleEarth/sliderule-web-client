...
const logger = createLogger('ParmUtils')
...
export function getGtsAndTracksWithGts(input_gts: SrListNumberItem[]): {
  gts: SrListNumberItem[]
...
export function getScOrientFromSpotGt(spot: number, gt: number) {
  ...
  if (spot === 1) {
    if (gt === 60) {
      ...
      logger.debug('getScOrientFromSpotGt: INVALID combo', { spot, gt })
    }
  } else if (spot === 2) {
    if (gt === 50) {
      scOrient = 0
...
export function mapGtStringsToSrListNumberItems(beams: (string | number)[]): SrListNumberItem[] {
  return gtsOptions.filter((gt) => beams.includes(gt.label) || beams.includes(gt.value))
}

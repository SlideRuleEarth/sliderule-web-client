...
export function createWhereClause(reqId: number) {
  ...
  if (func === 'atl03sp') {
    ...
    if (use_rgt_in_filter && rgt >= 0) {
      conds.push(`${quoteIdent(utfn)} = ${safeInt(rgt)}`)
    }

    // cycles/orbits
    if (cycles.length > 0) {
      conds.push(`${quoteIdent(uofn)} IN (${safeIntList(cycles)})`)
    }
    ...
    if (use_y_atc_filter && y_atc_is_valid && selected_y_atc !== undefined) {
      conds.push(
        `(y_atc BETWEEN ${safeNum(selected_y_atc - y_atc_margin)} AND ${safeNum(selected_y_atc + y_atc_margin)})`
      )
    }
    ...
    if (spots.length > 0) {
      conds.push(`${quoteIdent(usfn)} IN (${safeIntList(spots)})`)
    }
    whereStr = joinWhere(conds)
  } else if (
    func === 'atl03vp' ||
    ...
    if (use_y_atc_filter && y_atc_is_valid && selected_y_atc !== undefined) {
      const min_y_atc = safeNum(selected_y_atc - y_atc_margin).toFixed(3)
      const max_y_atc = safeNum(selected_y_atc + y_atc_margin).toFixed(3)

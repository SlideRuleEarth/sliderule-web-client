...
async function getReqParamStore(
  reqId: number
): Promise<ReturnType<typeof useReqParamsStore> | undefined> {
...
const handleWorkerMsg = async (workerMsg: WorkerMessage) => {
  ...
  switch (workerMsg.status) {
    ...
    case 'aborted':
      ...
      break
    case 'server_msg':
      //logger.warn('handleWorkerMsg server_msg:',workerMsg.msg);
      if (workerMsg.msg) {
    ...
    case 'progress':
      ...
      if (workerMsg.progress) {
        ...
        if (workerMsg.msg) {
          requestsStore.setSvrMsg(workerMsg.msg)
          const targetMsg = workerMsg.progress.target_numSvrExceptions
            ? `/${workerMsg.progress.target_numSvrExceptions}`
            : ''
          requestsStore.setConsoleMsg(
            `Received ${workerMsg.progress.numSvrExceptions} ${targetMsg} notifications`
          )
        }
      }
      break
      ...
      if (workerMsg && workerMsg.req_id > 0) {
        ...
        try {
          ...
          await db.updateRequestRecord({
            req_id: workerMsg.req_id,
            svr_parms: serverReqStr,
          ...
          if (rc?.parentReqId && rc.parentReqId > 0) {
            ...
            if (newReqId != workerMsg.req_id) {
              logger.error('newReqId does not match workerMsg.req_id', {
                newReqId,
                workerReqId: workerMsg.req_id
              })
            }
          }
        ...
        try {
          ...
          if (rc && rc.parentReqId > 0) {
            // this was a Photon Cloud request
...
export function processAbortClicked() {
  if (worker) {
    ...
    logger.debug('abortClicked', {
      isAborting: serverStateStore.isAborting,
      isFetching: serverStateStore.isFetching
    })
...
async function runFetchToFileWorker(srReqRec: SrRequestRecord): Promise<void> {
  try {
    ...
    if (srReqRec.req_id) {
      ...
      const cmd = {
        ...
        sysConfig: {
          ...
          cluster: sysConfigStore.cluster,
          jwt: accessToken
        },
        func: srReqRec.func,
        parameters: srReqRec.parameters
      } as WebWorkerCmd
    ...
    logger.error('runFetchToFileWorker error', {
      error: error instanceof Error ? error.message : String(error)
    })
...
export async function processRunSlideRuleClicked(rc: SrRunContext | null = null): Promise<void> {
  if (
    !(
      useReqParamsStore().getMissionValue() === 'ICESat-2' &&
      useReqParamsStore().getIceSat2API().includes('atl13')
    )
  ) {
  ...
  if (srReqRec) {
    ...
    if (srReqRec.req_id) {
      ...
      if (!reqParamsStore.enableGranuleSelection) {
        // granule selection is not enabled
        if (
          !reqParamsStore.ignorePolygon &&
          (reqParamsStore.poly === null || reqParamsStore.poly.length === 0) &&
          useGeoJsonStore().reqHasPoly() === false &&
          !(
            reqParamsStore.getMissionValue() === 'ICESat-2' &&
            reqParamsStore.getIceSat2API().includes('atl13')
          )
        ) {
          logger.warn('No geographic region defined', {
            poly: reqParamsStore.poly,
            ignorePolygon: reqParamsStore.ignorePolygon
          })
          if (rc === null) {
            ...
            logger.error('runSlideRuleClicked INVALID poly', {
              poly: reqParamsStore.poly,
              ignorePolygon: reqParamsStore.ignorePolygon
            })
          }
      ...
      if (reqParamsStore.getMissionValue() === 'ICESat-2') {
        if (reqParamsStore.getIceSat2API()) {
          ...
          if (reqParamsStore.getIceSat2API() === 'atl03' && srReqRec.parameters?.parms?.fit) {
            ...
            reqParamsStore.getIceSat2API() === 'atl03' &&
            srReqRec.parameters?.parms?.phoreal
          ) {

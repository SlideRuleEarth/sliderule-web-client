...
export class DuckDBClient {
  ...
  async query(query: string, params?: any): Promise<QueryResult> {
    ...
    try {
      ...
      return {
        schema,
        async *readRows(chunkSize = 10000) {
          // Default chunk size set to 100
  ...
  async getTotalRowCount(query: string): Promise<number> {
    ...
    try {
      ...
      logger.error('Error getting total row count', {
        error: error instanceof Error ? error.message : String(error)
      })
      throw error
    } finally {
  ...
  async queryForColNames(fileName: string): Promise<string[]> {
    ...
    try {
      ...
      try {
        logger.error('queryForColNames: DESCRIBE failed, trying pragma_table_info fallback', {
          fileName,
  ...
  async isParquetLoaded(name: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('isParquetLoaded error', {
        error: error instanceof Error ? error.message : String(error),
        name
      })
      return false
  ...
  async getJsonMetaDataForKey(
    ...
    formattedMetadata: string | undefined
    parsedMetadata: Record<string, any> | undefined
  }> {
    const conn = await this._db!.connect()
    ...
    try {
            ...
            if (keyString === key) {
              try {
                ...
                try {
                  ...
                  logger.error('getJsonMetaDataForKey Error stringifying JSON', {
                    keyString,
                    valueString,
                    parsedMetadata,
                    error:
                ...
                logger.error('getJsonMetaDataForKey Error parsing JSON', {
                  keyString,
                  valueString,
                  parsedMetadata,
              ...
              if (dumpAll) {
                try {
                  ...
                  logger.error('getJsonMetaDataForKey Error parsing JSON of other key', {
                    keyString,
                    error: parseError instanceof Error ? parseError.message : String(parseError)
                  })
                }
      ...
      logger.error('getJsonMetaDataForKey Error reading Parquet metadata', {
        error: error instanceof Error ? error.message : String(error)
      })
  ...
  async getAllParquetMetadata(
    ...
    try {
      ...
      logger.error('Error reading Parquet metadata', {
        parquetFilePath,
        error: error instanceof Error ? error.message : String(error)
      })
      throw error

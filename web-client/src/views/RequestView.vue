...
async function loadParametersFromRequest(reqId: number) {
  ...
  try {
    ...
    logger.debug('Retrieved request from DB', {
      reqId,
      hasRequest: !!request,
      hasRcvdParms: !!request?.rcvd_parms
    })
    ...
    if (
      ...
      Object.keys(request.parameters.parms).length > 0
    ) {
      parameters = request.parameters.parms
      logger.debug('Falling back to request.parameters', {
        paramCount: Object.keys(parameters).length
      })
    }
    ...
    if (request && parameters && Object.keys(parameters).length > 0) {
      ...
      if (request.func) {
        ...
        if (parameters.asset?.includes('icesat2') || func.startsWith('atl')) {
          ...
          if (
            func === 'atl03x-surface' ||
            (func === 'atl03x' && (parameters.fit || parameters.parms?.fit))
          ) {
            reqParamsStore.setIceSat2API('atl03x-surface')
          } else if (
            func === 'atl03x-phoreal' ||
            (func === 'atl03x' && (parameters.phoreal || parameters.parms?.phoreal))
          ) {
        ...
        logger.debug('Set mission and API', {
          mission: reqParamsStore.getMissionValue(),
          api: reqParamsStore.getIceSat2API() || reqParamsStore.getGediAPI()
        })
      }
    ...
    logger.error('Failed to load parameters for request', {
      reqId,
      error: error instanceof Error ? error.message : String(error)
    })
    toast.add({
